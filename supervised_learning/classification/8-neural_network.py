...
class NeuralNetwork:
    ...
    def __init__(self, nx, nodes):
        ...
        # Initialize weights of hidden layer with random normal distribution
        # Shape: (nodes, nx) - nodes neurons, each with nx weights
        self.W1 = np.random.randn(nodes, nx)

        # Initialize bias of hidden layer with zeros
        # Shape: (nodes, 1) - one bias per neuron in hidden layer
        self.b1 = np.zeros((nodes, 1))

        # Initialize activated output of hidden layer to 0
        self.A1 = 0

        # Initialize weights of output neuron with random normal distribution
        # Shape: (1, nodes) - 1 output neuron, with nodes weights
        # (one of each hidden neuron)
        self.W2 = np.random.randn(1, nodes)

        # Initialize bias of output neuron to 0
        self.b2 = 0

        # Initialize activated output of output neuron to 0
        self.A2 = 0
